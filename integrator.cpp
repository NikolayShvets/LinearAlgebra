...
long double TDormandPrinceIntegrator::Run(TModel* model)
{
	...
	while (t < t1){
		...
		//if(N%5000==0) cout << e << "|" << t << endl;
		//формируем результат плотной выдачей
		while((t_out < t + h)&&(t_out <= t1)){
			long double theta = (t_out - t)/h, b[6];
			b[0] = theta*(1+theta*(-1337.0/480+theta*(1039.0/360+theta*(-1163.0/1152))));
			b[1] = 0.0;
			b[2] = 100*powl(theta,2)*(1054.0/9275+theta*(-4682.0/27825+theta*(379.0/5565)))/3;
			b[3] = -5*powl(theta,2)*(27.0/40+theta*(-9.0/5+theta*(83.0/96)))/2.0;
			b[4] = 18225*powl(theta,2)*(-3.0/250+theta*(22.0/375+theta*(-37.0/600)))/848;
			b[5] = -22*powl(theta,2)*(-3.0/10+theta*(29.0/30+theta*(-17.0/24)))/7;
		...
		model->do_thing(X, t);
		//cout<<"v==> "<<model->test_nu<<" time==>"<<model->T<<endl;
		//cout<<model->T<<endl;
		//test_file<<model->T<<endl;
		//if (N % 50==0) {
